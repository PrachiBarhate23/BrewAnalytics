...
- **BERT Model**: A fine-tuned `bert-base-uncased` model for Positive/Neutral/Negative classification.
...
- **Engine**: Uses **STL Decomposition** (Seasonal-Trend decomposition via LOESS) and **ARIMA(1,1,1)** for high-precision time-series forecasting.
- **Features**: Capture growth trends, weekly seasonality (e.g., weekend rushes), and monthly peak/low detection.
- **Outlet Projections**: Individual ARIMA models for each of the 9 campus outlets.
...
- **Python 3.9+**
- **Node.js v18+**
- **Hardware**: At least 8GB RAM (BERT model requires ~500MB+ for inference).
...
cd e:/BrewAnalytics/BrewAnalytics/backend
...
```
*The server will run at `http://127.0.0.1:8000`. It may take a few seconds to load the BERT model on the first request.*

### Step 2: Frontend Setup (Vite + React)
The frontend provides the visual dashboard and interactive tools.
```bash
# 1. Open a new terminal in the root directory
cd e:/BrewAnalytics/BrewAnalytics
...
1. **Sales Forecasting**: Navigate to the "Sales Forecasting" page. Change the **Horizon** (30d/60d/90d) to see the ARIMA model adjust its projections and confidence bands.
...
3. **Live AI Sentiment**: Type a complex review in the "Sentiment Analysis" page to see the BERT model handle mixed sentiments in real-time.
